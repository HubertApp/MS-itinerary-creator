...
TEST_F(AStarTest, CheckReconstructPathIsPureReturnGreatValue) {
  std::unordered_map<std::string, std::string> cf{
      {"B", "A"}, {"C", "B"}, {"D", "C"}};
  EXPECT_EQ(astar::reconstruct_path(cf, "D"), astar::reconstruct_path(cf, "D"));
}
...
TEST_F(AStarTest, CheckReconstructPathIncompletePathReturnAtFirstOrigin) {
  std::unordered_map<std::string, std::string> came_from = {{"C", "B"},
                                                            {"D", "C"}};
...
TEST_F(AStarTest, CheckRunAstarUnreachableGoal) {
  EXPECT_FALSE(
      astar::run_astar(g, "A", "Z", astar::heuristic_zero).has_value());
}
...
TEST_F(AStarTest, CheckRunAstarHeuristicAdmissibleDoesNotInflateCost) {
  ...
  auto rh =
      astar::run_astar(g_admissible, "A", "D", astar::heuristic_haversine);
